...
ace.define("ace/tern/tern_server",["require","exports","module","ace/range","ace/lib/dom"], function (acequire, exports, module) {
    "use strict";
    ...
    function WorkerServer(ts, workerClass) {
        var blob = new Blob([ ts.options.workerScript ], { type: 'application/javascript' });
        var blobUrl = (window.URL || window.webkitURL).createObjectURL(blob);

        var worker = workerClass ? new workerClass() : new Worker(blobUrl);
...
ace.define("ace/tern/tern",["require","exports","module","ace/config","ace/lib/lang","ace/snippets","ace/autocomplete/text_completer","ace/autocomplete/util","ace/autocomplete","ace/tern/tern_server","ace/editor"], function(acequire, exports, module) {
    ...
    var doLiveAutocomplete = function(e) {
        ...
        if (e.command.name === "backspace") {
            ...
            if (!editor.completer) {
                ...
                else if (editor.$enableBasicAutocompletion) { //tern not enabled
                    editor.completers.push(textCompleter, keyWordCompleter);
                }
    ...
    var TernServer = acequire("./tern_server").TernServer;
    var aceTs;
    ...
    var editors_for_OnCusorChange = [];
    var getOnCursorChange_TernIndexByEditor = function (editor) {
        for (var i = 0; i < editors_for_OnCusorChange.length; i++) {
            if (editor === editors_for_OnCusorChange[i].editor) {
                return i;
            }
        }
        return null;
    };
    var getOnCursorChange_Tern = function (editor) {
        var debounceArgHints;
        return function (e, editor_getSession_selection) {
                ...
                if (val) {
                    ...
                    createTernServer(function() {
                        ...
                        self.on('destroy', function() {
                            ...
                            self.ternServer.server.closeAllTooltips();
                        });
                    ...
                    var onCursorChangeIndex = getOnCursorChange_TernIndexByEditor(self);
                    if (onCursorChangeIndex !== null) {
                        self.getSession().selection.off('changeCursor', editors_for_OnCusorChange[onCursorChangeIndex].handler);
                        editors_for_OnCusorChange.splice(onCursorChangeIndex, 1);
                    }
